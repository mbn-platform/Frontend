...
class ContractSettings extends React.Component {
  ...
  onFieldEdit = (e, interval) => {
    ...
    const isNewValueInInterval =
      (value >= interval[0])
      && (interval.length === 2 ? value <= interval[1] : true);

    if (interval) {
      if (isNewValueInInterval || value === '') {
  ...
  onEditButtonClick = () => {
    ...
    if (isEditing) {
      ...
      if (fee >= 100 || fee <= 0 || minAmount < 0 || roi <= 0 || duration <= 0 || maxLoss <= 0) {
      ...
      {this.state.isEditing ? (
        <div className="col-12 d-flex align-items-center justify-content-between choose-btn-group">
          <button
            ...
            type="button"
            className="edit-btn cancel-btn btn btn-secondary"
          >
            <FormattedMessage
              id="profile.cancel"
          ...
          <button
            ...
            type="button"
            className="edit-btn send-request-btn btn btn-secondary active"
          >
            <FormattedMessage
              id="profile.save"
              defaultMessage="SAVE"
...
const SettingEntry = ({ value, dimension }) => (
  <div className="value-text loss-text-block text-block">
    {value} <span className="days">{dimension}</span>
  </div>
);

const Setting = ({
  className, header, value, dimension,
  isEditing, editValue, onChange, name, tabIndex,
}) => (
...
const SettingAmount = ({
  isEditing, value, dimension, editValue,
  onChange, tabIndex, editCurrency,
}) => (
...
export const EditAmountEntry = ({
  placeholder, value, onChange, tabIndex, dimension,
}) => (
...
const EditSettingsEntry = ({
  className, placeholder, value,
  dimension, name, onChange, tabIndex,
}) => (
